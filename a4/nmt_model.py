...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size=hidden_size, bidirectional=True, device=dev)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, lengths_unpacked = pad_packed_sequence(enc_hiddens, batch_first=True)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        assert e_t.shape == (next_hidden.shape[0], enc_hiddens_proj.shape[1])
        ...
        assert a_t.shape == (next_hidden.shape[0], enc_hiddens.shape[2])

        U_t = torch.cat((next_hidden, a_t), dim=1)
        V_t = self.combined_output_projection(U_t)
        O_t = self.dropout(torch.tanh(V_t))
        assert O_t.shape == (next_hidden.shape[0], self.hidden_size)
