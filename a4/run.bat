...
) else if "%1%"=="train_eng2chr" (
    set CUDA_VISIBLE_DEVICES=0 & python run.py train --train-src=./chr_en_data/train.en --train-tgt=./chr_en_data/train.chr --dev-src=./chr_en_data/dev.en --dev-tgt=./chr_en_data/dev.chr --vocab=vocab.json --cuda --lr=5e-4 --patience=1 --valid-niter=200 --batch-size=32 --dropout=.3 --switch
) else if "%1%"=="test" (
