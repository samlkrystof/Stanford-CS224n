...
Options:
    ...
    --switch                                switch between the source and target language
"""
import math
import sys
import pickle
...
def train(args: Dict):
    ...
    """
    # changes translation direction if args contain --switch_translation
    che2eng = "--switch" in args
    ...
    if che2eng:
        ...
        train_data_src = read_corpus(args['--train-src'], source='src', vocab_size=8000)
        train_data_tgt = read_corpus(args['--train-tgt'], source='tgt', vocab_size=21000)

        dev_data_src = read_corpus(args['--dev-src'], source='src', vocab_size=2000)
    ...
    vocab = Vocab.load(args['--vocab'], che2eng)
