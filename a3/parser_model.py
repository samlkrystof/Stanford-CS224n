...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        first = nn.functional.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        first = self.dropout(first)
        logits = torch.matmul(first, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
